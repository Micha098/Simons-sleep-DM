...
output_folder =  f'/mnt/home/mhacohen/ceph/data/Empatica/AC/{user_name}/' 
...
def main(file, folderInn, folderOut, filesf):
    ...
    c_1s = pd.DataFrame(data = {'x' : c1_1s, 'y' : c2_1s, 'z' : c3_1s})
    c_1s = c_1s.astype(int)
    c_1s = c_1s.resample('30S', convention = 'start').sum()
    
    df = pd.read_csv(FileName, index_col = 'Unnamed: 0')
    df['time'] = df['time'].apply(lambda x: datetime.datetime.utcfromtimestamp(x))
    df.resample('30S', convention = 'start').sum()
    
    c_1s = pd.cocnat([df,c_1s],axis=1)
    
    # write to output folder
    fileOut = folderOut + New_fileName
...
New_fileName = f'ac{files[i][12:]}'
df = pd.read_csv(FileName, index_col = 'Unnamed: 0')
df['time'] = df['time'].apply(lambda x: datetime.datetime.utcfromtimestamp(x))
df.set_index('time',drop=True,inplace=True)


main(files[i], folderInn = input_folder, folderOut = output_folder,  filesf = 64)