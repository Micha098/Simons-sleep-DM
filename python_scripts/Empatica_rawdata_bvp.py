...
def process_and_save_adjusted_days(subject_id,tzs_str,output_folder,shared_data_folder,file1, file2=None):
    ...
    for datei in df_combined['day'].unique():
        new_filename = f'{output_folder}/empatica_bvp_{subject_id}_{datei}.csv'
...
def bvp_raw_data(i):
    ...
    tz_temp = f'/mnt/home/mhacohen/ceph/Sleep_study/SubjectsData/raw_data/harmonized_data/{subject_id[i]}/bvp/tz_temp' #output folder

    output_folder = f'/mnt/home/mhacohen/ceph/Sleep_study/SubjectsData/raw_data/harmonized_data/{subject_id[i]}/bvp/' #output folder
    shared_data_folder = f'/mnt/home/mhacohen/ceph/Sleep_study/SubjectsData/data_share/{subject_id[i]}/empatica/raw_data/bvp/' #output folder
    ...
    for date in [d for d in sorted(dates) if pd.to_datetime(d).date() > pd.to_datetime('2023-10-30').date()]:# date of starting the sleep study pilot
        ...
        if True:#(f'empatica_bvp_{subject_id[i]}_{date}.csv') not in sorted(os.listdir(output_folder),reverse=True) or (subject_id[i] in [133,333,310,110,147,347,115,315,140,340,158,358,159,359,161,361]):
    ...
    day_files = sorted([f for f in os.listdir(tz_temp) if f.startswith('empatica_bvp_') and f.endswith('.csv')])
    if len(day_files) > 0:
