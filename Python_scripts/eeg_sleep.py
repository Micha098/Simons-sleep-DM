    ...
    def get_manual_stages(self,stages_path):
        ...
        *** Consider changing to getting a list instead of dataframes with different column names (for each study)
        ...
        if self.project_name == 'NSRR':
            ...
            annot = mne.Annotations(onset=onset_seconds,
...
#                 return np.nan
    ...
    def preprocess_eeg_data(self, file_path, stages_path, preload=True, l_freq=0.40, h_freq=30):
        ...
        # self.create_epochs not relevent for Dreem because theres no manual scoring
            ...
            self.preprocessed_data.set_annotations(annotations)
            self.croppedData = self.raw_data
            
            return self.epochs,self.croppedData

    def compute_Alg_stages(self, croppeData, info, token, fileName):
        df_pred = pd.DataFrame()
        # selectedChannelsPat = re.compile(r'\w{3} \w\w-[M]\w')
        # ch_names = list(filter(selectedChannelsPat.match,info['ch_names']))
        if (self.project_name == 'NSRR') | (self.project_name == 'NSRR_Stages'):
            ch_names = croppeData.info['ch_names'][:-2]
        else:
            ch_names = croppeData.info['ch_names']
        for i in ch_names:
            sls = yasa.SleepStaging(croppeData, eeg_name = i)
            y_pred = sls.predict()
    ...
    def compute_noise_matrices(self, preprocessed_data, info, sd_crt = 1.5):
        ...
        if True:
            ...
            for eleci in range(median_var_all.shape[0]):
                delta_mark_all[:, eleci] = np.where(fft_delta[:,eleci] > sd_crt * sd_delta_all[eleci] + median_delta_all[eleci], 1, 0)
                theta_mark_all[:, eleci] = np.where(fft_theta[:,eleci] > sd_crt * sd_theta_all[eleci] + median_theta_all[eleci], 1, 0)
                alpha_mark_all[:, eleci] = np.where(fft_alpha[:,eleci] > sd_crt * sd_alpha_all[eleci] + median_alpha_all[eleci], 1, 0)
                beta_mark_all[:, eleci] = np.where(fft_beta[:,eleci] > sd_crt * sd_beta_all[eleci] + median_beta_all[eleci], 1, 0)            
                ...
                var_mark_all[:, eleci] = np.where((var_epoch_all[:,eleci] > sd_crt * sd_var_all[eleci] + median_var_all[eleci]) |
    ...
    def noise_summary(self,delta_mark_all, theta_mark_all, alpha_mark_all, beta_mark_all, kurt_mark_all, var_mark_all, mob_mark_all, comp_mark_all, amp_mark_all, diff_mark_all, maxk_amp_all, ch_names):
        ...
        ***applies to NSRR data!***
        ...
        # # Concatenate arrays
        mark_all = np.concatenate((delta_mark_all[:, :, np.newaxis], theta_mark_all[:, :, np.newaxis],
    ...
    def SoWu_algo(self, info, fftWelch, rejected_epochs):
        ...
        ratioAT_smoothed = gaussian_filter(ratioAT, sigma=10, mode = 'wrap')
        ...
        if self.project_name == 'NSRR':
            ...
            # peaks, _ = find_peaks(delta_smoothed[ind_g_100:], width = 20, height = minAmp, distance = 60)
...
#                 local_minimaAT.append(after_min)
    ...
    def SoWu_algo_check_conditions(self, info, fftClean, rejected_epochs, nan_flag, prom_flag):
        ...
        # get power of relevant frequencies
        power_all = fftClean.iloc[:,allfreqidx[0]:allfreqidx[1]+1]
...
#                             break
        ...
        if (so_algo==0) | (so_algo is None):
            print('failed loop so')            
            try:
                # define sleep onset as the last epoch prior to the forst delta local minima with ratio > threshold
                so_algo = strt + np.where(ratioAT[strt:local_minima[0]] >= target_value)[0][-1]
                print('logic')
                so_flag = 'logic'
            except:  # if there's no epoch that answers this criteria use local mnima as SO
                try:
                    # define sleep onset as the point og minimal ratio before the first delta peak
                    so_algo = strt + np.argmax(ratioAT[strt:local_minima[0]])+1
                    so_flag = 'ATlocal_min'
                    print('local min')
                except:
        ...
        # ID the sleep onset found precedes the first delta local minima by more than 20 minutes, redefine sleep onset as the first delta local minima        
        if so_algo <= max(0,local_minima[0] - 40):
            so_algo = local_minima[0]
            so_flag = 'local_minima2'
                
        ### find Final awaknning based on alpha/theta ratio -----
        
        target_value = (np.median(alpha_smoothed[peaks[0]-30:peaks[0]+30])+np.std(alpha_smoothed[peaks[0]-30:peaks[0]+30]))
        ...
        try:
            ...
            # # find last clean 10 consecutive epochs
            # last_clean = np.where(np.diff(clean_indices)==1)[0][-10:]
            
            clean_indices = np.where(noisevals==0)[0]
            # get differences between indices 
            diff_indices = np.diff(clean_indices)
            for i in range(len(diff_indices), 0, -1):
                if (diff_indices[i-10:i]==1).all():
                    # first_clean = i-9
                    wu_algo = clean_indices[i]
                    wu_flag = 'new_logic'
                    break
...
#                 for i in range(clean_indices[first_clean], noisevals.shape[0]-5):
...
#                 wu_flag = 'new_logic'
        ...
        if wu_algo is None:
            ...
            try: #define final awakening as the last epoch with ratio < threshold
                # wu_algo = local_minimaAT[-2] +np.where(ratioAT[local_minimaAT[-2]:end_wu] >= target_value)[0][0]
                wu_algo = local_minimaAT[-2] +np.where(ratioAT[local_minimaAT[-2]:end_wu] <= target_value)[0][-1] #change to this! 30/10
                wu_flag = 'logic'
                print('logic')
            except:
                try:
        ...
        return so_algo, wu_algo, delta_smoothed, ratioAT, alpha_power, delta_power, theta_power, peaks, so_flag,  wu_flag, ratioAT_smoothed
